...
import { AuthProvider, CourierAvailability, UserRole, UserStatus, } from "../../generated/prisma/enums";
...
export const seedUsers = async () => {
    try {
        ...
        if (!defaultZone) {
            ...
            console.log(`📍 Created default zone: ${defaultZone.name} (${defaultZone.code})`);
        }
        ...
        if (!defaultHub) {
            ...
            console.log(`🏢 Created default hub: ${defaultHub.name} (${defaultHub.code})`);
        }
        ...
        for (const userData of usersToSeed) {
            ...
            if (existingUser) {
                ...
                if (userData.role === UserRole.COURIER && !existingUser.courier) {
                    ...
                    console.log(`📦 Created missing Courier profile for: ${userData.email}`);
                }
                continue;
            }
            ...
            if (userData.role === UserRole.COURIER) {
                ...
                console.log(`✅ Seeded ${userData.role}: ${userData.email} (with Courier profile)`);
            }
            else {
                await prisma.user.create({
                    data: {
