...
const createShipmentRequest = async (userId, payload) => {
    ...
    if (!payload.pickupAddress && !payload.pickupAddressId) {
        throw new AppError(httpStatus.BAD_REQUEST, "Pickup address is required. Please fill in pickup address details to add the address for your account.");
    }
    // 3. Validate delivery address requirement
    if (!payload.deliveryAddress && !payload.deliveryAddressId) {
        throw new AppError(httpStatus.BAD_REQUEST, "Delivery address is required. Please fill in delivery address details.");
    }
